...
POTENTIAL_MATCHES AS (
    ...
    SELECT 
        ...
        other.HUMIDITY as match_humidity,
        other.WIND_SPEED as match_wind_speed,
...
SELECT 
    ...
    MAX(CASE WHEN p.target_offset = -12 THEN p.match_humidity END) as HUM_LAG_12,
    MAX(CASE WHEN p.target_offset = -12 THEN p.match_wind_speed END) as WIND_LAG_12,
    ...
    MAX(CASE WHEN p.target_offset = -24 THEN p.match_humidity END) as HUM_LAG_24,
    MAX(CASE WHEN p.target_offset = -24 THEN p.match_wind_speed END) as WIND_LAG_24,
    ...
    MAX(CASE WHEN p.target_offset = -48 THEN p.match_humidity END) as HUM_LAG_48,
    MAX(CASE WHEN p.target_offset = -48 THEN p.match_wind_speed END) as WIND_LAG_48,
